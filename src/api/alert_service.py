...
from src.notifications.alert_dispatcher import send_alert as send_telegram_alert, send_patient_alert, send_photo_alert, send_video_alert
...
class AlertService:
    ...
    def send_video_alert(self, alert: Alert, video_path: str, roles: List[str] = None) -> bool:
        """Send alert with video via configured methods"""
        # Check cooldown
        if self._is_in_cooldown(alert):
            self.logger.info(f"Alert {alert.alert_type} for patient {alert.patient_id} is in cooldown")
            return False
        ...
        # Log alert
        self.logger.info(f"🚨 VIDEO ALERT: {alert.alert_type}")
        self.logger.info(f"   Patient ID: {alert.patient_id}")
        self.logger.info(f"   Room: {alert.room_id}")
        self.logger.info(f"   Frame: {alert.frame_number}")
        self.logger.info(f"   Time: {alert.timestamp.strftime('%H:%M:%S')}")
        ...
        try:
            ...
            # Format message for video alert
            message = self._format_alert_message(alert)
            
            for role in target_roles:
                try:
                    role_success = loop.run_until_complete(
                        send_video_alert(role, message, video_path, self._get_priority_from_alert_type(alert.alert_type))
                    )
                    if not role_success:
                        self.logger.warning(f"Failed to send video alert to role: {role}")
                        success = False
                    else:
                        self.logger.info(f"Video alert sent successfully to role: {role}")
                except Exception as e:
                    self.logger.error(f"Error sending video alert to role {role}: {e}")
                    success = False
            
            loop.close()
            
        except Exception as e:
            self.logger.error(f"Error in video alert sending process: {e}")
            success = False
